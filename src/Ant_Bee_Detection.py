...
def make_net():
    ...
    model = nn.Sequential(
        # 第一卷积块（提取底层特征）
        nn.Conv2d(in_channels=3, out_channels=6, kernel_size=3),  # 输入3通道，输出6通道，3x3卷积核
        nn.ReLU(inplace=True),  # 激活函数（原地计算节省内存）
        nn.MaxPool2d(kernel_size=2, stride=2),  # 2x2最大池化，步长2（特征图尺寸减半）

        # 第二卷积块（提取中层特征）
        nn.Conv2d(6, 16, 5),
        nn.ReLU(True),
        nn.MaxPool2d(2, 2),  # 再次池化降维

        # 第三卷积块（捕获高层语义特征）
        nn.Conv2d(16, 120, 5),  # 最终卷积层输出120通道
        nn.ReLU(True),
        ...
        # 分类头
        nn.Linear(in_features=120, out_features=84),  # 全连接层（120 → 84维特征）
        nn.ReLU(True),
        nn.Linear(in_features=84, out_features=2)  # 最终输出层（84 → 2类logits）
    )
...
def plot_loss_accuracy(train_losses, val_losses, train_accs, val_accs):
    ...
    # 保存图片
    plt.savefig('loss_accuracy_curve.png', dpi=300, bbox_inches='tight')
