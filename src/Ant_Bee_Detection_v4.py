...
dataset_transforms = {
    ...
    'train': transforms.Compose([
        ...
        transforms.RandomHorizontalFlip(p=0.8),  # 提高二分类（对称生物）的增强概率
        transforms.GaussianBlur(kernel_size=(3, 7), sigma=(0.1, 2.0)),  # 增强抗模糊能力
        transforms.RandomRotation(45),  # 随机旋转45度
        transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3),  # 增强颜色抖动
        transforms.AutoAugment(),  # 增加增强多样性
    ...
    'val': transforms.Compose([
        transforms.Resize(256, antialias=True),  # 将图像短边缩放至256像素（保持长宽比）
...
def make_net():
    """ 使用预训练的ResNet18模型 """
    # 加载预训练模型（权重基于ImageNet）
    model = models.resnet18(weights=ResNet18_Weights.DEFAULT)

    # 冻结所有卷积层参数（可选，根据数据集大小决定）
...
def plot_learning_rate(lrs):
    ...
    # 保存图表到本地文件
    plt.savefig('learning_rate_curve_v4.png', dpi=300, bbox_inches='tight')
...
def plot_loss_accuracy(train_losses, val_losses, train_accs, val_accs):
    ...
    # 保存图片
    plt.savefig('loss_accuracy_curve_v4.png', dpi=300, bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    # ------------------ 定义优化器 ------------------
    optimizer = optim.AdamW(  # 使用AdamW代替Adam（更好的权重衰减处理）
        model.parameters(),
        lr=1e-4,  # 初始学习率降低（微调需求）
        weight_decay=1e-4
    )

    # ------------------ 定义学习率调度器 ------------------
    scheduler = optim.lr_scheduler.CosineAnnealingLR(  # 余弦退火调度器
        optimizer,
        T_max=30,  # 周期长度（通常设为epoch总数的一半）
        eta_min=1e-6  # 最小学习率
    )

    # ------------------ 执行模型训练 ------------------
    model = train_model(model, criterion, optimizer, scheduler, num_epochs=60)

    # ------------------ 保存最佳模型 ------------------
    torch.save({'model': model, 'state_dict': model.state_dict()}, 'best_model_v4.pth')
